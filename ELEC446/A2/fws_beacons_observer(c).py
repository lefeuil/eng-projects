"""
KB_luen_observer(c).py
For ELEC 446: question 1.c) of assignment 2. 
Simulates observer of a diffdrive robot on a circular path based on ranges from 2 beacons.
Observations: estimated angle seems to lag behind actual angle. 
    This is possibly because the robot's orientation is difficult 
    to estimate based on ranges (ie. centre-point location) alone; 
    therefore, the angle is estimated based on last position vs current,
    introducing lag.
...
for k in range(1, N):
    ...
    x_hat[:, k] = diff_observer(x_hat[:, k - 1], u[:, k - 1], r, f_map)
